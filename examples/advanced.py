...
def main():
    instance = MultimodalDataPipeline(config={
        "timeout": 30,
        "retries": 3,
        "debug": True,
    })

    print("=== Advanced multimodal-data-pipeline Example ===\n")

    # Batch processing
    inputs = [f"data_{i}" for i in range(10)]
    results = [instance.learn(input=inp, index=i) for i, inp in enumerate(inputs)]
